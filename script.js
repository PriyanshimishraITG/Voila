...
shopNew.addEventListener("mouseenter", () => {
    ...
    hideTimeout = setTimeout(() => {
        (hContentNew.style.display = "none")
    }, 2000);
});
...
hContentNew.addEventListener("mouseleave", () => {
    (hContentNew.style.display = "none")});

const searchIconDivNew = document.querySelector(".searchIconDiv");
searchIconDivNew.addEventListener("click", () => {
    (checkOnSearch());
});
let hideTimeout;
...
const searchInputHTML = `
    <div style="display: flex; align-items: center; justify-content: center; text-align: center; width: 50%; gap: 25px;">
        ...
        <div class="setIcon cursor alignC" id="closeSearch" onclick="closeSearch()">
            <p> x </p>
        </div>
    </div> `;
...
//     if (!(headerDiv.contains(event.target)) || (event.target.closest("#closeSearch"))) {
//         (closeSearch())
//     };
// };
...
function closeSearch() {
    ...
    headerDiv.classList.toggle("reduced-padding");
    searchScrollDiv.style.display = "none";
